        ...
        columns: [
          ...
          },
          {
            ...
            Cell: ({ cell }) => (
              <Fragment>
                <NavLink
                  className='view_source_link'
                  to={`/source/${cell.value}`}
                >
                  View Source{' '}
                </NavLink>
                {props.valid == true ? (
                  <Fragment>
                    <NavLink
                      className='view_source_link edit_source_link'
                      to={`/editsource/${cell.row.values._id}`}
                    >
                      Edit
                    </NavLink>

                    <NavLink
                      className='view_source_link delete_source_link'
                      to={`/deletesource/${cell.row.values._id}`}
                    >
                      Delete
                    </NavLink>
                  </Fragment>
    ...
    const fetchUsers = async () => {
      try {
        ...
        const responseData = await sendRequest(
          `https://invest-mate-backend.vercel.app/api/source/getcompany/${gid}`,
          'POST'
        );
