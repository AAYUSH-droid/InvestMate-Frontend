        ...
        columns: [
          ...
          },
          {
            ...
            Cell: ({ cell }) => (
              <Fragment>
                <NavLink
                  className='accept_source_link edit_source_link'
                  to={`/request/${cell.row.values._id}/${true}`}
                >
                  Accept{' '}
                </NavLink>{' '}
                &nbsp;
                <NavLink
                  className='accept_source_link delete_source_link'
                  to={`/request/${cell.row.values._id}/${false}`}
                >
                  Reject{' '}
                </NavLink>
              </Fragment>
    ...
    const fetchUsers = async () => {
      try {
        const responseData = await sendRequest(
          `https://invest-mate-backend.vercel.app/api/source/approve/${userId}`,
          'POST'
        );
