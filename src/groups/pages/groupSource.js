...
const JoinGroupAuth = () => {
  ...
  const letlev = async (e) => {
    ...
    try {
      ...
      body = JSON.stringify(body);
      const responseData = await sendRequest(
        `https://invest-mate-backend.vercel.app/api/groups/remove/${userId}/`,
        'POST',
        body,
        {
          'Content-Type': 'application/json',
  ...
  return (
    <React.Fragment>
      ...
      <div className='group_form_div'>
        <center>
          <NavLink className='request_btns' to={`/requestsource/${gid}`}>
            Request/Add Source
          </NavLink>
          <NavLink className='request_btns' to={`/groupdetails/${gid}`}>
            Member Details
          </NavLink>
