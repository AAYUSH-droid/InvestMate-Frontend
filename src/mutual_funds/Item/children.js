        ...
        {headerGroups.map((headerGroup) => (
          <tr {...headerGroup.getHeaderGroupProps()}>
            {headerGroup.headers.map((column) => (
              <th className='join_group_head' {...column.getHeaderProps()}>
        ...
        {rows.map((row, i) => {
          ...
          return (
            <tr className='join_group_row' {...row.getRowProps()}>
              {row.cells.map((cell) => {
                return (
                  <td className='join_group_data' {...cell.getCellProps()}>
                    {cell.render('Cell')}
                  </td>
                );
              })}
    ...
    const fetchUsers = async () => {
      try {
        ...
        const responseData = await sendRequest(
          `https://invest-mate-backend.vercel.app/Api/source/getcatalogue/`,
          'POST',
          body,
          {
    ...
    <Fragment>
      <div className='group_form_div'>
        <center>
          ...
          >
            <h2 className='search_header'>Filter Funds by</h2>
            <hr />
            ...
            <select
              className='search_input'
              name='purc_all'
              onChange={(e) => setPurc_all(e.target.value)}
            >
              <option></option>
            ...
            <input
              ...
              onChange={(e) => setMin_pur_amnt(e.target.value)}
            />
            <br />
            ...
            <select
              className='search_input'
              name='divd_type'
              onChange={(e) => setDivd_type(e.target.value)}
            >
              <option></option>
              <option value='idcw-reinvest' className='options'>
                idcw-reinvest
              </option>
              <option value='growth' className='options'>
                growth
              </option>
              <option value='idcw-payout' className='options'>
                idcw-payout
              </option>
            </select>
            <br />
            ...
            >
              <option></option>
              <option value='regular' className='options'>
                regular
              </option>
            ...
            <select
              className='search_input'
              name='setll_ty'
              onChange={(e) => setSetll_ty(e.target.value)}
            >
              <option></option>
              <option value='T1' className='options'>
                T1
              </option>
              <option value='T2' className='options'>
                T2
              </option>
              <option value='T3' className='options'>
                T3
              </option>
              <option value='T4' className='options'>
                T4
              </option>
              <option value='T5' className='options'>
                T5
              </option>
              <option value='T6' className='options'>
                T6
              </option>
              <option value='T7' className='options'>
                T7
              </option>
            </select>
            <br /> <br />
