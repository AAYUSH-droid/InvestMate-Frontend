...
function GroupMembers() {
  ...
  const mkadm = async (e) => {
    try {
      ...
      const responseData = await sendRequest(
        `https://invest-mate-backend.vercel.app/api/groups/transferownership/removegroup/${userId}/`,
        'POST',
        body,
        {
          ...
          },
          {
            ...
            Cell: ({ cell }) => (
              <Fragment>
                {cell.value === userId ? (
                  <button
                    className='make_admin'
                    button
                    onClick={() => mkadm(cell.row.original.userId._id)}
                  >
                    Make Admin
                  </button>
                ) : cell.row.original.userId._id === cell.value ? (
                  <h5>Group Owner</h5>
                ) : (
                  <h5>Member</h5>
                )}
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const responseData = await sendRequest(
          `https://invest-mate-backend.vercel.app/api/groups/groupmembers/${gid}`,
          'POST'
        );
