    ...
    () => [
      {
        ...
        columns: [
          ...
          },
          {
            ...
            accessor: '_id',
            Cell: (e) => (
              <NavLink className='join_group_link' to={`/yourgroup/${e.value}`}>
                {' '}
                Click here{' '}
              </NavLink>
            ),
    ...
    const fetchUsers = async () => {
      try {
        const responseData = await sendRequest(
          `https://invest-mate-backend.vercel.app/api/groups/getgroups/${userId}`,
          'POST'
        );
      ...
      <div className='group_form_div'>
        <center>
          <form
            ...
            id='event_form'
            name='event_form'
            className='my_group_search_form'
            onSubmit={onSubmitform}
          >
            <h2 className='search_header'>Filter Groups by</h2>
            <hr />
            ...
            >
              <option></option>
              <option value='Gold/Silver' className='options'>
                Gold/Silver
              </option>
              <option value='Stock' className='options'>
                Stock
              </option>
              <option value='Cryptocurrency' className='options'>
                Cryptocurrency
              </option>
              <option value='Currency Exchange' className='options'>
                Currency Exchange
              </option>
            </select>
            &nbsp;
            ...
            >
              ...
              <option value='6' className='options'>
                6 Months
              </option>
              <option value='12' className='options'>
                1 Year
              </option>
              <option value='60' className='options'>
                5 Years
              </option>
            </select>
            &nbsp;
            ...
            <ul className='group_source_links'>
              <Link to={`/getgroupsource/${userId}`}>
                <button className='group_source_button'>
                  Manage Group Source
                </button>
              </Link>
              <Link to={`/ownedgroups/getgroups/${userId}`}>
                <button className='group_source_button'>
                  Manage Owned Groups
                </button>
              </Link>
