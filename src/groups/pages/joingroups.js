...
const JoinGroupAuth = () => {
  ...
  const onSubmitform = async (e) => {
    ...
    try {
      ...
      const responseData = await sendRequest(
        `https://invest-mate-backend.vercel.app/api/groups/join/${userId}`,
        'POST',
        body,
        {
  ...
  return (
    <React.Fragment>
      {error ? (
        <ErrorModal error={success} onClear={successHandler} />
      ) : (
      ...
      <div className='group_form_div'>
        <center>
          ...
          >
            {/* form header */}
            <h2 className='form_heading'>JOIN THIS GROUP</h2>
            <input
              type='number'
              name='amount'
              className='join_inputs'
              value={amount}
              step='50'
              min='50'
